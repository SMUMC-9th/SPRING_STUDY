...
public class OAuth2ServiceImpl implements OAuth2Service {
    ...
    public MemberResponseDTO.LoginResponseDTO login(String code) {
        ...
        try {
            tokenResponse = kakaoAuthClient.getToken("authorization_code", clientId, redirectURI, code, clientSecret);
        } catch (Exception e) {
        ...
        try {
            oAuth2TokenDTO = objectMapper.readValue(tokenResponse, OAuth2DTO.OAuth2TokenDTO.class);
        } catch (Exception e) {
            throw new MemberException(MemberErrorCode.OAUTH_TOKEN_FAIL);
        }
        ...
        try {
            userInfoResponse = kakaoUserClient.getUserInfo("Bearer " + oAuth2TokenDTO.getAccess_token());
        } catch (Exception e) {
        ...
        try {
            profile = objectMapper.readValue(userInfoResponse, OAuth2DTO.KakaoProfile.class);
        } catch (Exception e) {
            throw new MemberException(MemberErrorCode.OAUTH_USER_INFO_FAIL);
        }
        ...
        if (member == null) {
            // 이메일로 못 찾은 경우, 혹시 username으로 찾을 수 있는지 확인 (데이터 불일치 방지)
